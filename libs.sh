...
function fetch_h264() {
    ...
    if [ ! -d $LIB_NAME ]; then
        ...
        curl -O "https://download.videolan.org/pub/videolan/x264/snapshots/x264-snapshot-20191217-2245-stable.tar.bz2"
        tar -xf "x264-snapshot-20191217-2245-stable.tar.bz2"
        mv x264-snapshot-20191217-2245-stable $LIB_NAME
        rm x264-snapshot-20191217-2245-stable.tar.bz2
    else
...
function build_h264() {
    ...
    ./configure --prefix=${BUILD_DIR}/external/${ANDROID_ABI_PARAM} \
    --host=${TARGET} --sysroot=${SYSROOT_PATH} \
    --enable-pic --enable-static --enable-shared --disable-cli --disable-avs --disable-lavf \
    --disable-cli --disable-ffms --disable-opencl --chroma-format=all \
    --bit-depth=all --disable-asm
...
function build_openssl() {
    ...
    # ./Configure $TARGET_OPENSSL_ARM no-shared -D__ANDROID_API__=21 -I${ANDROID_NDK_HOME}/sysroot/usr/include -I${ANDROID_NDK_HOME}/sysroot/usr/include/arm-linux-androideabi --prefix=$INSTALL_DIR
